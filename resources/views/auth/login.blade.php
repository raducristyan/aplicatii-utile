...
<form class="" id="loginform" method="POST" action="{{ route('login') }}">
    ...
    <div class="flex flex-wrap items-center mt-6">
        <div class="flex w-full items-center">
            ...
            <input id="email" class="w-full bg-transparent border-b-2 border-purple-400 focus:border-gray-300 px-2 py-3 text-gray-200 font-normal outline-none {{ $errors->has('email') ? ' border-b-2 border-red-600' : '' }}" name="email" type="email" required="" placeholder="Introduceți adresa de email" value="{{ old('email')}}">
        </div>
        @if ($errors->has('email'))
        <div class="w-full text-sm font-normal text-red-600 mt-1">
            {{ $errors->first('email') }}
        </div>
        @endif
    </div>
    <div class="flex items-center mt-6">
        ...
        <input id="password" class="w-full bg-transparent border-b-2 border-purple-400 focus:border-gray-300 px-2 py-3 text-gray-200 font-normal outline-none {{ $errors->has('email') ? ' border-b-2 border-red-600' : '' }}" type="password" name="password" required="" placeholder="Introduceți parola">
    </div>
