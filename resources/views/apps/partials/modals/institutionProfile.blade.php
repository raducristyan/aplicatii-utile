<institution-profile-view email="{{ $institution->email ?? ''}}" name="{{ $institution->name ?? ''}}" cif="{{ $institution->cif ?? ''}}" bank="{{ $institution->bank ?? ''}}" iban="{{ $institution->iban ?? ''}}" phone="{{ $institution->phone ?? ''}}" fax="{{ $institution->fax ?? ''}}" institution_id="{{ $institution->id ?? ''}}" url="{{ route('admin.institution.profile') }}" v-show="modalIsOpen('edit-institution-profile-modal')" @close-modal="closeModal" :modal-is-open="modalIsOpen('edit-institution-profile-modal')" inline-template>
    ...
    <button type="button" class="bg-red-100 text-red-500 font-medium border-b-2 border-red-500 px-3 py-2 hover:border-red-600 hover:bg-red-500 focus:underline-none focus:shadow-outline hover:text-gray-100 rounded-sm shadow-md mr-0" @click="closeModal('edit-institution-profile-modal')">
        Renunță
    </button>
    @endslot
    @endcomponent
